
import React, { useState } from "react";
import logo from "../assets/logo.jpg"

export default function Navbar({ fixed }) {
  const [navbarOpen, setNavbarOpen] = useState(false);

  const onToggleMenu = () => {
    setNavbarOpen(!navbarOpen);
  };

  return (
    <>
      <div className="font-[Poppins] bg-gradient-to-t from-[#fbc2eb] to-[#a6c1ee] h-screen">
        <header className="bg-white">
          <nav className="flex justify-between items-center w-[92%] mx-auto">
            <div>
              <img
                className="w-16 cursor-pointer"
                // src="https://cdn-icons-png.flaticon.com/512/5968/5968204.png"
                src={logo}
                alt="..."
              />
            </div>
            <div className={`nav-links duration-500 md:static absolute bg-white md:min-h-fit min-h-[60vh] left-0 top-${navbarOpen ? '9%' : '-100%'} md:w-auto w-full flex items-center px-5`}>
              <ul className="flex md:flex-row flex-col md:items-center md:gap-[4vw] gap-8">
                <li>
                  <a className="hover:text-gray-500" href="javascript:void(0)">Products</a>
                </li>
                <li>
                  <a className="hover:text-gray-500" href="javascript:void(0)">Solution</a>
                </li>
                <li>
                  <a className="hover:text-gray-500" href="javascript:void(0)">Resource</a>
                </li>
                <li>
                  <a className="hover:text-gray-500" href="javascript:void(0)">Developers</a>
                </li>
                <li>
                  <a className="hover:text-gray-500" href="javascript:void(0)">Pricing</a>
                </li>
              </ul>
            </div>
            <div className="flex items-center gap-6">
              <button className="bg-[#a6c1ee] text-white px-5 py-2 rounded-full hover:bg-[#87acec]">Sign in</button>
              <ionicon onClick={onToggleMenu} name={navbarOpen ? 'close' : 'menu'} className="text-3xl cursor-pointer md:hidden"></ionicon>
            </div>
          </nav>
        </header>
      </div>
    </>
  );
}
